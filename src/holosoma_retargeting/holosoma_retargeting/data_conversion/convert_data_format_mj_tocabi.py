...
class MotionLoader:
    ...
    def _load_motion(self):
        ...
        if self.motion_file.endswith(".npz"):
            ...
            self.input_fps = int(np.asarray(data.get("fps", self.input_fps)).item())
            self.input_dt = 1.0 / self.input_fps
...
def run_simulator(args_cli: TocabiDataConversionConfig):
    ...
    if (
        ...
        motion_config = TocabiMotionDataConfig(
...
def main(args_cli: TocabiDataConversionConfig):
    """Main function."""
    # Run the simulator
...
if __name__ == "__main__":
    ...
    tyro_config = tyro.cli(TocabiDataConversionConfig)
    main(tyro_config)
