...
from holosoma_retargeting.config_types.tocabi import (  # noqa: E402
    TocabiMotionDataConfig,
    TocabiRetargetingConfig,
    TocabiRobotConfig,
)
from holosoma_retargeting.src.interaction_mesh_retargeter_tocabi import (  # noqa: E402
...
def create_task_constants(
    robot_config: TocabiRobotConfig,
    motion_data_config: TocabiMotionDataConfig,
...
def validate_config(cfg: TocabiRetargetingConfig) -> None:
...
def build_retargeter_kwargs_from_config(
    ...
    if getattr(constants, "ROBOT_NAME", "").startswith("tocabi"):
...
def main(cfg: TocabiRetargetingConfig) -> None:
    ...
    if cfg.robot_config.robot_type != robot:
        cfg.robot_config = TocabiRobotConfig()

    if cfg.motion_data_config.robot_type != robot or cfg.motion_data_config.data_format != data_format:
        cfg.motion_data_config = TocabiMotionDataConfig(data_format=data_format)
...
if __name__ == "__main__":
    cfg = tyro.cli(TocabiRetargetingConfig)
    main(cfg)
