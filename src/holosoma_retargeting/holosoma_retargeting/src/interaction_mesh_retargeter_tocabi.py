...
class InteractionMeshRetargeter:
    ...
    def solve_single_iteration(
        ...
        if apply_foot_sticking or apply_foot_lock:
            ...
            if apply_foot_sticking:
                ...
                for key, J_WF in J_WF_dict.items():
                    key_lower = key.lower()
                    apply_left = (key_lower.startswith("l_") or "left" in key_lower) and foot_sticking[left_key]
                    apply_right = (key_lower.startswith("r_") or "right" in key_lower) and foot_sticking[right_key]
        ...
        # Self-collision constraints. Keep these soft so a short collision
        # window cannot make the whole SQP step infeasible.
        self_collision_slacks = []
        ...
        for key, phi in phis_sc.items():
            ...
            slack = cp.Variable(nonneg=True, name=f"self_collision_slack_{len(self_collision_slacks)}")
            constraints += [Ja_n @ dqa + slack >= rhs]
            self_collision_slacks.append(slack)
        ...
        if self_collision_slacks:
            obj_terms.append(1e4 * cp.sum_squares(cp.hstack(self_collision_slacks)))
    ...
    def _prefilter_pairs_with_mj_collision(self, threshold: float):
        ...
        ground_ids = [g for g, name in enumerate(self._geom_names) if "ground" in name]
        object_ids = []
        if self.object_name != "ground":
            object_ids = [g for g, name in enumerate(self._geom_names) if self.object_name in name]

        anchor_ids = set(ground_ids + object_ids)

        # Non-penetration only needs robot-vs-ground/object candidates. Running
        # global mj_collision can fail on dense robot-robot mesh contacts.
        candidates = set()
        for anchor in anchor_ids:
            for other in range(ngeom):
                if other == anchor:
                    continue
                if other in anchor_ids:
                    continue
                if m.geom_contype[other] == 0 and m.geom_conaffinity[other] == 0:
                    continue
                if m.geom_contype[anchor] == 0 and m.geom_conaffinity[anchor] == 0:
                    continue
                g1, g2 = sorted((anchor, other))
