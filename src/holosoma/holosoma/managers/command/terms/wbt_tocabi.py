"""Tocabi-specific Whole Body Tracking command term.

This keeps the original G1 WBT command implementation intact while adding
Tocabi body-name aliases for converted MuJoCo motion files.
"""
...
from holosoma.managers.command.terms.wbt import MotionCommand as _BaseMotionCommand
...
class MotionLoader:
    ...
    def _get_index_of_a_in_b(self, a_names: List[str], b_names: List[str], device: str = "cpu") -> torch.Tensor:
        indexes = []
        for name in a_names:
            motion_name = FAKE_BODY_NAME_ALIASES.get(name, name)
            assert motion_name in b_names, f"The specified name ({name} -> {motion_name}) doesn't exist: {b_names}"
            indexes.append(b_names.index(motion_name))
...
## Helper functions
#########################################################################################################
TOCABI_BODY_NAME_ALIASES: dict[str, str] = {
    # Tocabi retargeting uses these semantic foot names in some configs, but
    # the converted MuJoCo motion currently only exports ankle bodies.
    "L_Foot_Link": "L_AnkleRoll_Link",
    "R_Foot_Link": "R_AnkleRoll_Link",
    "left_foot_contact_point": "L_AnkleRoll_Link",
    "right_foot_contact_point": "R_AnkleRoll_Link",
}
